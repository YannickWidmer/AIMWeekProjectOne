function data = loadContentDH(filename)
...
%% Read columns of data as strings:
% For more information, see the TEXTSCAN documentation.
formatSpec = '%9s%15s%9s%15s%15s%13s%3s%4s%7s%s%[^\n\r]';
...
rawNumericColumns = raw(:, [1,2,3,4,5,9,10]);
rawCellColumns = raw(:, [6,7,8]);

data = cell2mat(rawNumericColumns(:, 1:5));
